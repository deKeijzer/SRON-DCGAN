...
import model_v5_small as model
...
if use_saved_weights:
    try:
        # Load saved weights
        netG.load_state_dict(torch.load('gan_data//weights//netG_state_dict_wgan_model_v5_small', map_location=device))
        netD.load_state_dict(torch.load('gan_data//weights//netD_state_dict_wgan_model_v5_small', map_location=device))
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        if (iters % 100 == 0): # save weights every % .... iters
            #print('weights saved')
            if ngpu > 1:
                torch.save(netG.module.state_dict(), 'gan_data//weights//netG_state_dict_wgan_model_v5_small')
                torch.save(netD.module.state_dict(), 'gan_data//weights//netD_state_dict_wgan_model_v5_small')
            else:
                torch.save(netG.state_dict(), 'gan_data//weights//netG_state_dict_wgan_model_v5_small')
                torch.save(netD.state_dict(), 'gan_data//weights//netD_state_dict_wgan_model_v5_small')
        ...
        if (iters % (64) == 0) and (iters != 0):
            ...
            for z,variable in enumerate(variables_to_save):
                save_progress('v5_test_wgan', variable_names[z], variable)
