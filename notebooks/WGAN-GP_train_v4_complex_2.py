...
if use_saved_weights:
    try:
        # Load saved weights, 'netG_simple_1' to start from pretrained model
        netG.load_state_dict(torch.load('gan_data//weights//netG_complex_2', map_location=device))
        netD.load_state_dict(torch.load('gan_data//weights//netD_complex_2', map_location=device))
        print('Succesfully loaded saved weights.')
    except:
        print('Could not load saved weights, using new ones.')
        pass
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        if (iters % 128 == 0): # save weights every % .... iters
            print('weights saved')
            if ngpu > 1:
                torch.save(netG.module.state_dict(), 'gan_data//weights//netG_complex_22')
                torch.save(netD.module.state_dict(), 'gan_data//weights//netD_complex_22')
            else:
                torch.save(netG.state_dict(), 'gan_data//weights//netG_complex_22')
                torch.save(netD.state_dict(), 'gan_data//weights//netD_complex_22')
