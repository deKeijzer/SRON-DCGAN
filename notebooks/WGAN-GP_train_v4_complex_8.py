...
if use_simple_weights:
    try:
        ...
        netG.load_state_dict(torch.load('gan_data//weights//netG_simple_8', map_location=device))
        netD.load_state_dict(torch.load('gan_data//weights//netD_simple_8', map_location=device))
...
if use_saved_weights:
    try:
        # Load saved weights, 'netG_simple_1' to start from pretrained model
        netG.load_state_dict(torch.load('gan_data//weights//netG_complex_8', map_location=device))
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        if (iters % 128 == 0): # save weights every % .... iters
            print('weights saved')
            if ngpu > 1:
                torch.save(netG.module.state_dict(), 'gan_data//weights//netG_complex_8')
                torch.save(netD.module.state_dict(), 'gan_data//weights//netD_complex_8')
            else:
                torch.save(netG.state_dict(), 'gan_data//weights//netG_complex_8')
                torch.save(netD.state_dict(), 'gan_data//weights//netD_complex_8')
