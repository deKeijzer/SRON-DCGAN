...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        if (iters % 100 == 0): # save weights every % .... iters
            #print('weights saved')
            if ngpu > 1:
                torch.save(netG.module.state_dict(), 'gan_data//weights//netG_state_dict0_v4_test_gan')
                torch.save(netD.module.state_dict(), 'gan_data//weights//netD_state_dict0_v4_test_gan')
            else:
                torch.save(netG.state_dict(), 'gan_data//weights//netG_state_dict0_v4_test_gan')
                torch.save(netD.state_dict(), 'gan_data//weights//netD_state_dict0_v4_test_gan')
