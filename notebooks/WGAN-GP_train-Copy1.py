...
if use_saved_weights:
    try:
        # Load saved weights
        netG.load_state_dict(torch.load('netG_state_dict00', map_location=device)) #net.module..load_... for parallel model , net.load_... for single gpu model
        netD.load_state_dict(torch.load('netD_state_dict00', map_location=device))
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        if (iters % 100 == 0): # save weights every % .... iters
            #print('weights saved')
            if ngpu > 1:
                torch.save(netG.module.state_dict(), 'netG_state_dict1')
                torch.save(netD.module.state_dict(), 'netD_state_dict1')
            else:
                torch.save(netG.state_dict(), 'netG_state_dict1')
                torch.save(netD.state_dict(), 'netD_state_dict1')
